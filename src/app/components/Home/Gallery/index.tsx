    ...
    <section id='gallery' className='scroll-mt-20'>
      <div className='container'>
        ...
        <div className='my-16 px-6'>
          <Masonry
            ...
            {loading
              ...
              : (galleryImages?.length > 12 ? galleryImages.slice(0, 12) : galleryImages).map((item, index) => (
                  <div
                    key={index}
        ...
        <div className='flex justify-center'>
          <Link
            href='/gallery'
            className='inline-block px-6 py-2 border border-primary rounded-full text-base font-medium text-white bg-primary hover:bg-primary/20 hover:text-primary hover:cursor-pointer transition ease-in-out duration-300'>
            {t('gallery.button.view')}
          </Link>
          {/* menu pop-up */}
          {isMenuOpen && (
            <div
                ...
                <div className='max-h-[350px] overflow-y-auto'>
                  <table className='w-full table-auto border-collapse text-left'>
                    <thead className='sticky top-0 bg-neutral-100 z-10'>
                      <tr>
                        <th className='py-3 px-4'>{t('gallery.table.dish')}</th>
                        <th className='py-3 px-4'>{t('gallery.table.description')}</th>
                        <th className='py-3 px-4'>{t('gallery.table.price')}</th>
                      </tr>
                    </thead>
                    <tbody>
